...
if __name__ == '__main__':
    ...
    parser.add_argument('--rep_dir', type=str, default="/mnt/nfs3/zhangjinouwen/dataset/rep/small_wav2vec2")
    ...
    train_file_list = "/mnt/nfs3/zhangjinouwen/dataset/rep/rep_small_wav_train.txt"
    valid_file_list = "/mnt/nfs3/zhangjinouwen/dataset/rep/rep_small_wav_eval.txt"
    ...
    with torch.no_grad():
        for i, audio_file in tqdm(enumerate(file_list)):
            ...
            #     rep_file = audio_file.replace(str(path1), f"{args.rep_dir}/emilia").split('.')[0] +'.wavbert.npy'
            if str(path2) in audio_file:
                rep_file = audio_file.replace(str(path2), f"{args.rep_dir}/clean100").split('.')[0] +'.wavbert.npy'
            # else :
            #     rep_file = audio_file.replace(str(path3), f"{args.rep_dir}/clean360").split('.')[0] + '.wavbert.npy'
