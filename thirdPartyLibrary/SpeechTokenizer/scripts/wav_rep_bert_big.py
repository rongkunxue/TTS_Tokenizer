...
def build_semantic_model(device):
    ...
    stat_mean_var = torch.load("/root/Github/TTS_Tokenizer/thirdPartyLibrary/SpeechTokenizer/wav2vec2bert_stats.pt")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rep_dir', type=str, default="/mnt/nfs3/zhangjinouwen/dataset/rep/big_wav2vec2")
    ...
    train_file_list = "/mnt/nfs3/zhangjinouwen/dataset/rep/rep_big_wav_train.txt"
    valid_file_list = "/mnt/nfs3/zhangjinouwen/dataset/rep/rep_big_wav_eval.txt"
    ...
    with torch.no_grad():
        for i, audio_file in tqdm(enumerate(file_list)):
            ...
            if str(path1) in audio_file:
                rep_file = audio_file.replace(str(path1), f"{args.rep_dir}/emiliaen").split('.')[0] +'.wav.npy'
            elif str(path2) in audio_file:
                rep_file = audio_file.replace(str(path2), f"{args.rep_dir}/clean100").split('.')[0] +'.wav.npy'
            elif str(path4) in audio_file:
                rep_file = audio_file.replace(str(path2), f"{args.rep_dir}/emiliazh").split('.')[0] +'.wav.npy'
            else :
                rep_file = audio_file.replace(str(path3), f"{args.rep_dir}/clean360").split('.')[0] + '.wav.npy'
