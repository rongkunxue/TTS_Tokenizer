...
def main(args):
    ...
    LibriTTSTestClean_dataset = LibriTTSTestClean(data_path_str)
    test_clean_loader = utils.data.DataLoader(LibriTTSTestClean_dataset, batch_size=1, shuffle=False, num_workers=4, collate_fn=pad_collate_fn)
    ...
    with torch.no_grad():
        for batch in tqdm(test_clean_loader):
            assert batch["waveform"].shape[0] == 1
            result=[]
            sample_rate = 24000
            segments = batch["waveform"].new_zeros(1, ((batch["waveform"].size(1) + sample_rate - 1) // sample_rate) * sample_rate)
            segments[:, :waveform.size(1)] = waveform
            waveform = segments.view(1, -1, sample_rate)
            paths.append(batch["audio_path"][0])
            for i in range(waveform.size(1)):
                audio = waveform[:, i, :].to(DEVICE)
                if model.use_ema:
                ...
                for index in indices.flatten():
                    usage[index.item()] += 1
                result.append(reconstructed_audios)
            result = torch.cat(result, dim=1)
            reconstructed_audios = result
    ...
    for i in tqdm(range(len(paths))):
        rawwav,rawwav_sr=torchaudio.load(data_path_str, paths[i])
