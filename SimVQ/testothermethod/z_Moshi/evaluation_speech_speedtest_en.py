...
def main(args):
    ...
    mimi = loaders.get_mimi(mimi_weight, device='cuda:2')
    mimi.set_num_codebooks(8)
    codebook_size = 1024
    usage = {}
    for i in range(codebook_size):
        usage[i] = 0
    if not os.path.exists(f"{args.ckpt_path.parent}/recons/seedme.txt"):
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader):
                ...
                codes = mimi.encode(audio)  # [B, K = 8, T]
                decoded = mimi.decode(codes)
                generative_audio_path = os.path.join(f"{args.ckpt_path.parent}/recons/seed_test/{utt}.wav")
                directory = os.path.dirname(generative_audio_path)
                os.makedirs(directory, exist_ok=True)
                torchaudio.save(generative_audio_path, decoded[0].cpu().clip(min=-0.99, max=0.99), sample_rate=24000, encoding='PCM_S', bits_per_sample=16)
            ...
            with open(f"{args.ckpt_path.parent}/recons/seedme.txt", "w") as f:
                for path in paths:
                    f.write(path + "\n")
    else:
        paths = []
        f = open(f"{args.ckpt_path.parent}/recons/seedme.txt")
        lines = f.readlines()
    ...
    with open(Path(args.ckpt_path).parent / "seedme.txt", 'w') as f:
...
def get_args():
    ...
    parser.add_argument("--ckpt_path", default="/root/Github/TTS_Tokenizer/SimVQ/test/z_Moshi/a.pt", type=Path)
