    ...
    if not os.path.exists(f"{args.ckpt_path.parent}/recons/test_other.txt"):
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader):
                ...
                inputs = processor(raw_audio=audio_sample, sampling_rate=processor.sampling_rate, return_tensors="pt").to(DEVICE)
                ...
                torchaudio.save(generative_audio_path, audio_values[0].cpu().clip(min=-0.99, max=0.99), sample_rate=24000, encoding='PCM_S', bits_per_sample=16)
...
def get_args():
    ...
    parser.add_argument("--ckpt_path", default="//root/Github/TTS_Tokenizer/SimVQ/test/Endoec/a.pt", type=Path)
