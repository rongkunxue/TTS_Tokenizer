...
def main(args):
    model = dac.DAC.load("/root/Github/TTS_Tokenizer/SimVQ/test/DAC_4/weights_24khz_8kbps_0.0.4.pth").to(DEVICE)
    model.eval()
    codebook_size = 1024
    usage = {}
    for i in range(codebook_size):
        usage[i] = 0
    if not os.path.exists(f"{args.ckpt_path.parent}/recons/seed.txt"):
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader):
                ...
                x = model.preprocess(audio, 24000).to(DEVICE)
                z, codes, latents, _, _ = model.encode(x,n_quantizers=4)
                y = model.decode(z)

                generative_audio_path = os.path.join(f"{args.ckpt_path.parent}/recons/seed/{utt}.wav")
                directory = os.path.dirname(generative_audio_path)
                os.makedirs(directory, exist_ok=True)
                torchaudio.save(generative_audio_path, y[0].cpu().clip(min=-0.99, max=0.99), sample_rate=24000, encoding='PCM_S', bits_per_sample=16)
            ...
            with open(f"{args.ckpt_path.parent}/recons/seed.txt", "w") as f:
                for path in paths:
                    f.write(path + "\n")
    else:
        paths = []
        f = open(f"{args.ckpt_path.parent}/recons/seed.txt")
        lines = f.readlines()
    ...
    with open(Path(args.ckpt_path).parent / "seed_result.txt", 'w') as f:
...
def get_args():
    ...
    parser.add_argument("--ckpt_path", default="/root/Github/TTS_Tokenizer/SimVQ/test/DAC_4/a.pt", type=Path)
