...
def main(args):
    ...
    if not os.path.exists(f"{args.ckpt_path.parent}/recons/seedtest_paths.txt"):
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader):
                ...
                reconstructed_audios = model.decode(codes[0: (0 + 1)], st=0) 
                generative_audio_path = os.path.join(f"{args.ckpt_path.parent}/recons/seedtest_paths/{utt}.wav")
    ...
    for i in tqdm(range(len(paths))):
        ...
        prewav_24k=torchaudio.functional.resample(prewav, orig_freq=rawwav_sr, new_freq=24000)
...
def get_args():
    ...
    parser.add_argument("--ckpt_path", default="/root/Github/TTS_Tokenizer/SimVQ/test/speech_1/a.pt", type=Path)
