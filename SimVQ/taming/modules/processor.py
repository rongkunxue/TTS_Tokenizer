...
class FBank(object):
    ...
    def __call__(self, wav_batch, dither=0):
        sr = 16000
        assert sr == self.sample_rate, f"Expected sample rate {self.sample_rate}, but got {sr}"
        assert wav_batch.shape[1] == 1, "Input must be single-channel audio."

        batch_size = wav_batch.shape[0]
        feats = []

        for i in range(batch_size):
            wav = wav_batch[i, 0, :].unsqueeze(0)  # 取出第 i 个样本，形状变为 [1, samples]
            feat = Kaldi.fbank(
                wav, 
